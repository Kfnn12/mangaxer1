...
export function AZView({ letter, onSelectManga }: AZViewProps) {
  ...
  const fetchAZ = useCallback(() => {
    setLoading(true);
    setError(null);
    api.getAZList(letter)
      .then(setData)
      .catch((e) => {
         setError(e.message);
         toast.error("Failed to load A-Z list text", { description: e.message });
      })
  ...
  if (!data?.results || data.results.length === 0) {
    return (
      <div className="flex flex-1 flex-col items-center justify-center text-zinc-500">
        ...
        <p className="mt-2 text-sm">We couldn't find anything matching letter "<span className="text-orange-500">{letter}</span>"</p>
      </div>
    );
  }
    ...
    <section className="flex-1 flex flex-col gap-8">
      <div className="flex items-center justify-between mb-2">
        <h2 className="text-xl font-black italic tracking-wider flex items-center gap-2">
          ...
          A-Z LIST: <span className="text-orange-600">{letter}</span>
        </h2>
      </div>
      <div className="grid grid-cols-2 gap-6 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
        {data.results.map((manga, i) => (
          <MangaCard key={`az-${manga.id}-${i}`} manga={manga} onClick={onSelectManga} />
        ))}
      </div>
    </section>
  );
